...
Run test file:
$ pytest tests/test_neural_entropy.py -—cov=pybor.neural_entropy

Or to save as html report:
$ pytest tests/test_neural_entropy.py -—cov=pybor.neural_entropy --cov-report=html

Or to report line numbers of missing coverage:
$ pytest tests/test_neural_entropy.py --cov=pybor.neural_entropy --cov-report term-missing

Or to perform a specific test:
$ pytest tests/test_neural_entropy.py::test_prediction2 --cov=pybor.neural_entropy --cov-report term-missing
...
from pybor.config import RecurrentSettings
from pybor.neural_data import NeuralData
from pybor.neural_entropy import NeuralEntropyRecurrent, NeuralEntropyRecurrentAttention
from pybor.neural_entropy import NeuralEntropyMultiheadAttention
...
def test_instantiation():
    ...
    word = NeuralEntropyRecurrent(data.vocab.size)
    ...
    word = NeuralEntropyRecurrent(data.vocab.size, settings=settings)
    assert word.settings.print_summary is True
...
def test_2recurrent_layers():
    ...
    settings = RecurrentSettings(rnn_levels=2, rnn_cell_type="LSTM")
    word = NeuralEntropyRecurrent(data.vocab.size, settings=settings)
...
def test_settings():
    ...
    neural = NeuralEntropyRecurrent(data.vocab.size, settings=settings)
    assert neural.settings.val_split == 0.20
    assert neural.settings.verbose == 0
    assert neural.settings.token_maxlen == 30
...
def test_train_model():
    ...
    word = NeuralEntropyRecurrent(data.vocab.size, language="German")
    word.train(train_gen=data.trainer, val_gen=data.validator)
    word.evaluate_test(test_gen=data.tester)
...
def get_mean_entropies(data, neural):
    ...
    fit_mean_entropy = statistics.mean(neural.calculate_entropies(fit_tokens_ids))
    val_mean_entropy = statistics.mean(neural.calculate_entropies(val_tokens_ids))
    test_mean_entropy = statistics.mean(neural.calculate_entropies(test_tokens_ids))
    print("***")
    print(
        f"Entropies: fit {fit_mean_entropy:.4f}, "
...
def test_train_model_with_cfg():
    ...
    settings = RecurrentSettings(
        rnn_cell_type="LSTM", epochs=10, embedding_dropout=0.1, val_split=0.15
    )

    data = NeuralData(training1, testing1, settings=settings)
    word = NeuralEntropyRecurrent(data.vocab.size, language="German", settings=settings)
    word.train(train_gen=data.trainer, val_gen=data.validator)
    word.evaluate_test(test_gen=data.tester)
    get_mean_entropies(data, word)


def test_train_model_with_cfg_attn():
    ...
    data = NeuralData(training1, testing1, settings=settings)
    word = NeuralEntropyRecurrentAttention(data.vocab.size, language="German", settings=settings)
    word.train(train_gen=data.trainer, val_gen=data.validator)
    word.evaluate_test(test_gen=data.tester)
    get_mean_entropies(data, word)


def test_train_model_with_cfg_mhattn():

    settings = RecurrentSettings(
        rnn_cell_type="MHATTN", epochs=45, embedding_dropout=0.1, val_split=0.15,
        tf_verbose=1, verbose=2
    )

    data = NeuralData(training1, testing1, settings=settings)
    neural = NeuralEntropyMultiheadAttention(data.vocab.size, language="German", settings=settings)
    neural.train(train_gen=data.trainer, val_gen=data.validator)
    neural.evaluate_test(test_gen=data.tester)
    get_mean_entropies(data, neural)

    settings.positional_encoding = 'TRIG'
    data = NeuralData(training1, testing1, settings=settings)
    neural = NeuralEntropyMultiheadAttention(data.vocab.size, language="German", settings=settings)
    neural.train(train_gen=data.trainer, val_gen=data.validator)
    neural.evaluate_test(test_gen=data.tester)
...
if __name__ == "__main__":
    # test_instantiation()
    # test_2recurrent_layers()
    # test_settings()
    # test_train_model()
    # test_train_model_with_cfg()
    # test_train_model_with_cfg_attn()
    test_train_model_with_cfg_mhattn()
