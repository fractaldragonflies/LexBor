...
def calculate_fscore(n_true_pos, n_true, n_pos, beta=1.0):
    if n_true_pos == 0:
        return 0.0
    if n_true == 0:
        return 0.0
...
# =============================================================================
# Partitioner for use in k-fold cross-validation
# =============================================================================
def k_fold_samples(table, k, k_iter=None):
    size = len(table)
    table = random.sample(table, size)
    fraction = 1 / k if k > 1.0 else 0.0
    k_iter = k if k_iter is None else k_iter
...
# =============================================================================
# Partitioner for use in holdout-n cross-validation
# =============================================================================
...
# =============================================================================
# Partitioner for use in tri-part k-fold cross-validation
# =============================================================================
# def k_fold_samples_3part(table, cv):
#     #
#     table_size = len(table)
#     fraction = 1 / cv
#     table = random.sample(table, table_size)
#
#     i = 0
#     while i < cv:
#         # Break out train and test from table
#         test_begin = math.ceil(i * table_size * fraction)
#         test_end = math.ceil((i + 1) * table_size * fraction)
#         test = table[test_begin:test_end]
#         train = table[:test_begin] + table[test_end:]
#         # Break out fit and val from train
#         train_size = len(train)
#         train = random.sample(train, train_size)
#         val_end = math.ceil(train_size * fraction)
#         fit = train[:-val_end]
#         val = train[-val_end:]
#         yield i, fit, val, test
#         i += 1
#
